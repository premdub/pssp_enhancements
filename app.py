...
from models import db, Users, Patients, Conditions_patient, Conditions, Medications_patient, Medications, Procedures, Procedures_Patient, Patients_Photos
...
def register_admin():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            msg = 'Account already exists !  Please login using your username and password'   
            return render_template('/login.html', msg = msg)
        else:
...
def register_patient():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account or patient:
            ...
            new_user = Users(username, password, email, account_type, mrn, datecreated, lastlogin)
            new_patient = Patients(mrn, first_name, last_name, gender, dob, zip_code, contact_mobile, contact_home)
...
def update_procedures(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        patient_procedure = Procedures_patient.query.filter_by(id=form_id).first()
        print('patient_treatments_procedures', patient_procedure)
        patient_condition.cpt_code = request.form.get('cpt_code')
        db.session.commit()
...
def add_procedure(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        form_mrn = request.form.get('mrn')
        print('form_mrn', form_mrn)
        form_cpt = request.form.get('cpt_code')
        print('form_cpt_code', form_cpt)
        new_procedure = Procedures_patient(form_mrn, form_cpt)
        db.session.add(new_procedure)
        db.session.commit()
        flash("Patient Procedure Added Successfully")
        ## then return to patient details page
        return redirect(url_for('get_patient_details', mrn=form_mrn))
...
def delete_procedure(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        print('Found procedures: ', patient_procedure)
        for procedure in patient_procedure:
            db.session.delete(procedure)
        db.session.commit()
        flash("Patient Procedure Deleted Successfully")
        ## then return to patient details page
        return redirect(url_for('get_patient_details', mrn=form_mrn))
